...
public class PerlinNoiseController : MonoBehaviour
{
    ...
    [SerializeField] private GameObject _noiseObject;  //どのオブジェクトを動かすか

    private void Start()
    {
        // 初期位置を保持
        _initPosition = _noiseObject.transform.position;
    }
    ...
    private void Update()
    {
        ...
        // 揺れ位置情報更新
        _noiseObject.transform.position = GetUpdateShakePosition(
            _shakeInfo,
            _totalShakeTime,
            _initPosition);
        ...
        if (_totalShakeTime >= _shakeInfo.Duration)
        {
            ...
            // 初期位置に戻す
            _noiseObject.transform.position = _initPosition;
        }
    }
